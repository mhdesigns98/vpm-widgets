...
Each form POSTs directly to Mailchimp's native embedded-form endpoint for the Virginia Home Grown audience (`list-manage.com/subscribe/post`) — no custom API, no backend, no API key. This is Mailchimp's own documented embed pattern (Audience > Signup forms > Embedded forms).
